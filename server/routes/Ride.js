...
router.post("/processing", (req, res) => {
    ...
    if (
        ...
        pool.getConnection((err, con) => {
            if (err) throw err;
            con.query(
                `INSERT INTO temp_ride (temp_ride_session, name, email, phone, pickup, destination, payment) VALUES ('${temp_ride_session_id}','${req.body.name}','${req.body.email}','${req.body.phone}','${req.body.pick}','${req.body.dest}','${req.body.pay_mode}')`,
				...
				var mailOptions = {
					...
					text: `Name: ${req.body.name}\nPickup Location: ${req.body.pick}\nDrop Location: ${req.body.dest}\nPhone: ${req.body.phone}`
				};
                    ...
                    return res.status(200).send({
                        status: 200,
                        message:
                            "Request sent to the company. Driver will be here soon.",
                    });
                }
            );
        });
...
router.post("/searching", (req, res) => {
    ...
    pool.getConnection((err, con) => {
        if (err) throw err;
        con.query(
            `SELECT * FROM current_rides WHERE customer_id = '${req.body.customer_id}'`,
            function (err, result, fields) {
                con.release();
...
router.put("/searching", (req, res) => {
    ...
    if (
        ...
        pool.getConnection((err, con) => {
            if (err) throw err;
            con.query(
                `INSERT INTO rideRequests (request_id, customer_id, name, email, phone, pickup, destination, payment) VALUES ('${request_id}','${req.body.customer_id}','${req.body.name}','${req.body.email}','${req.body.phone}','${req.body.pickup}','${req.body.destination}','${req.body.payment}')`,
                function (err, result, fields) {
                    con.release();
                    pool.getConnection((err, con) => {
                        if (err) throw err;
                        con.query(
                            `DELETE FROM temp_ride WHERE temp_ride_session = '${sess.temp_session_id}'`,
                            function (err, result, fields) {
                                con.release();
...
router.post("/connected", (req, res) => {
    ...
    pool.getConnection((err, con) => {
        if (err) throw err;
        con.query(
            `Select * from current_rides WHERE ride_id = '${req.body.ride_session}'`,
            function (err, result, fields) {
                con.release();
